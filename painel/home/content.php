...
$data = date("Y-m-d");
$query = "select count(*) as qt, situacao from logs_conexoes where data like '%".(($_SESSION['conectividade_data'])?:$data)."%' and loja = '".(($_SESSION['conectividade_loja'])?:6)."' group by situacao";
...
<!-- Content Row -->
<?php
    $query = "select *, hour(data) as h, minute(data) as m, if(situacao = '0', 'red', 'green') as cor from logs_conexoes where data like '%".(($_SESSION['conectividade_data'])?:$data)."%' and loja = '".(($_SESSION['conectividade_loja'])?:6)."'";
    $result = mysqli_query($con, $query);
    while($d = mysqli_fetch_object($result)){
